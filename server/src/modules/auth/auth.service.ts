...
class UserService {
  ...
  signUp = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const ttl = await this.sendEmailOtp({ email, userName: user.userName });
    res
      .status(200)
      .json({ message: "User signed up successfully.", data: { ttl, email } });
  };
  ...
  resendOtp = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const ttl = await this.sendEmailOtp({
      email,
  ...
  login = async (req: Request, res: Response, next: NextFunction) => {
    const { email, password, fcm, rememberMe }: SignInDTO = req.body;

    const user = await this._userModel.findOne({ filter: { email } });
    if (!user) throw new AppError("User not exists", 409);
    if (!user.confirmed) {
      const ttl = await this.sendEmailOtp({ email, userName: user.userName });
      successResponse({
        res,
    ...
    const refresh_token = await this._tokenService.GenerateToken({
      payload: { id: user._id, email },
      secret_key: REFRESH_SECRET_KEY_USER,
      options: { expiresIn: rememberMe ? "30d" : "1d", jwtid },
    });
    res.cookie("refreshToken", refresh_token, {
      ...
      sameSite: "lax",
      maxAge: rememberMe
        ? 1000 * 60 * 60 * 24 * 30 // 30 يوم
        : 1000 * 60 * 60 * 24 * 1, // 1 day
    });
    // FCM token handling ----------->
    if (fcm) {
      await this._redisService.addFCM({ userId: user._id, FCMToken: fcm });
      const tokens = await this._redisService.getFCM(user._id);
  ...
  refreshToken = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { REFRESH_SECRET_KEY, ACCESS_SECRET_KEY } =
    await this._tokenService.getSignature(PREFIX_USER);

    const { decoded, user } =
      await this._tokenService.decodedToken_and_fetchUser(
        refreshToken,
        REFRESH_SECRET_KEY,
      );
