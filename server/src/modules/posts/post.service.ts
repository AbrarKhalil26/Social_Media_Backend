...
class PostService {
  ...
  async attachSignedUrls(post: any) {
    const postObj = post.toObject ? post.toObject() : post;
    if (Array.isArray(postObj.attachment) && postObj.attachment.length) {
      postObj.attachment = await Promise.all(
        postObj.attachment.map((key: string) =>
          this._s3Service.getPreSignedUrlByKey(key),
        ),
      );
    } else {
      postObj.attachment = [];
  ...
  async attachSignedUrlsToMany(posts: any[]) {
  ...
  createPost = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const postWithUrls = await this.attachSignedUrls(post);
    successResponse({
      res,
  ...
  getAllPosts = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const postsWithUrls = await this.attachSignedUrlsToMany(posts);
    successResponse({ res, data: postsWithUrls });
  };
  ...
  deletePost = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!post)
      throw new AppError(
        "Post not found or you not have permission to delete this post",
      );
